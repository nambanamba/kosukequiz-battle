...
const SHOTS = path.join(ROOT, "tools", "rewrite", "shots");
...
const units = await page.evaluate(ids =>
  [...new Set(QA_DATA.filter(q=>ids.includes(q.id)).map(q=>q.u))], TARGETS);
console.log("対象単元:", units.join(" / "));
...
for (const u of units) {
  ...
  console.log("  [" + u + "] 画面=" + await page.evaluate(()=>{
