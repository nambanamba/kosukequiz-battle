...
// ★★そのため、**①とBの枠は既定では走りません。**
//   　既定 … ②と③だけ（relay を使わない）
//   　`--with-relay` を付けたとき … ①とBも走る（★本物の relay に告知を出します）
//   ★飛ばしたときは「飛ばした・未確認」と出力に書きます。✔ とは言いません（4-2）。
//
//   ⚠️なぜ既定を外したか: ③の枠を試すつもりで丸ごと回し、relay を1回よけいに使ったため。
//   ★はじめは頭のコメントに「気をつけて」と書きましたが、**それは仕組みではありません**（0-3）。
//
...
// ==================== ①とB: 本物の待ち合わせ先（relay）を使う枠 ====================
// ★★ここから先は、既定では走りません（2026-09-26）。
//   ⚠️★理由: この道具は **②③だけ見たいときにも丸ごと回されます**。
//   　 実際に、③の枠を試すつもりで丸ごと回して、本物の relay を1回よけいに使いました。
//   ★頭のコメントに「気をつける」と書きましたが、**それは仕組みではありません**（確認ポイント 0-3）。
//   　 次に急いでいる人は、コメントを読まずに `node public_release_check.mjs` と打ちます。
//   → ★**頼まれたときだけ入る**形にしました。
//   ★そして「飛ばしたのに通ったように見える」ことがないよう、
//     飛ばしたときは**飛ばしたと出力に書き、✔ とは言いません**（4-2）。
const WITH_RELAY = process.argv.includes("--with-relay");
if (!WITH_RELAY) {
  await browser.close();
  console.log("\n■ ①とB（本物の待ち合わせ先を使う枠）は ★飛ばしました");
  console.log("  ★見ていません。見るには --with-relay を付けてください:");
  console.log("      node tools/mikaku/public_release_check.mjs --with-relay");
  console.log("  ⚠️ 本物の relay に告知を出すので、続けて何回も回さないこと");
  console.log("     （2026-09-20 に relay.damus.io で banned を食らった実績があります）");
  console.log(ng === 0
    ? "\n△ relay を使わない枠（②と③）は全部通りました。★①とBは**未確認**です"
    : "\n✘ " + ng + " 件ひっかかりました（relay を使わない枠だけで）");
