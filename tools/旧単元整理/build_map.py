...
# 括弧の直後に来る「種類を表す語」。これが違えば別のものを指している。
# 「が行われています」のような文の続きは、ここには入らない
KIND = {
    "市", "村", "町", "県", "府", "都", "道",
    "湾", "湖", "川", "山", "山地", "山脈", "平野", "盆地", "台地", "半島", "島", "諸島",
    "海", "高地", "高原", "用水", "港", "空港", "海岸", "地帯",
    "農業", "工業", "業", "駅", "工業地帯", "工業地域", "埠頭",
}
# 「（大津）市」と「（大津）」のように、新側で省かれることがある語
CITY = {"市", "村", "町"}
...
def after_compatible(oa, na):
    ...
    ★これが無いと「（愛知）県」と「（愛知）用水」が結ばれる（実際に起きた）。
      括弧の中が同じでも、直後が別の種類を指していれば別問題。

    ★ただし「種類を表す語かどうか」で分けないと行きすぎる。
      旧「（単作）が行われています」／新「（単作）地帯」は同じ事実なのに、
      単純な先頭一致だと落ちてしまう（claude-18 の再レビューで発覚）。
      「が行われています」は種類を表す語ではなく文の続きなので、そこは通す。

    ★「盆地」と「（空）」も通してはいけない。
      旧「（諏訪）盆地」と新「（諏訪）」＝諏訪市 が結ばれてしまう。
      ただし「市・村・町」だけは新側で省かれることがあるので例外として通す。
    """
    if oa and na:
        # どちらも種類を表す語で、違うものなら別物（県 vs 用水）
        if oa in KIND and na in KIND and oa != na:
            return False
        return True
    # 新側に種類の語が無いとき。旧が「盆地」なら別物、「市」なら同じものとみなす
    if not na and oa in KIND and oa not in CITY:
        return False
    return True
...
report = []
mapping = {}       # 旧id -> [新id, ...]
stats = {"旧問題": 0, "対応あり": 0, "対応なし": 0, "1対多で見送り": 0, "ゆるめの照合で拾えた": 0, "ゆるめでも決まらないので見送り": 0}
unmatched = []
matched_rows = []
...
for old_u, new_u in PAIRS.items():
    ...
    for o in olds:
        ...
        if not hits:
            ...
            # ★安全弁は「候補が1件のとき」ではなく「候補をしぼる道具」として先に使う。
            #   そうしないと、候補が2件あるだけで見送られる。
            #   例: 旧「（⑩甲府）市」の候補は kaki6_48「（甲府）」と kaki6_19「（甲府）盆地」の2件。
            #       安全弁が「盆地」を落とせば、残る1件で決まる
            oa = after_word(o.get("a", ""))
            ok = [c for c in cand if after_compatible(oa, after_word(c.get("a", "")))]
            if len(ok) == 1:
                hits = ok
                stats["ゆるめの照合で拾えた"] += 1
            elif cand:
                stats["ゆるめでも決まらないので見送り"] += 1
        if not hits:
            stats["対応なし"] += 1
